import {AppSvg, CustomSvg, PeopleSvg} from 'assets';

const services = [
  {
    Icon: CustomSvg,
    title: 'Customize System',
    content:
      'There are tons of systems around the world, some of them are not properly handled or maintained by engineers, poor implementation of codebase leads to poor performance and numerous issues, inaccurate applied architecture that result to become more complex of the system and tightly coupled system one of the great factor why migration/integration in the features are too expensive in time, these are commonly issue to other software developer team that has a poor foundation in system architecture.'
  },
  {
    Icon: AppSvg,
    title: 'Software Development',
    content:
      'I pursue myself to learn about new up coming technologies that trending on our dev community that gives me more knowledge on my specialize field as being frontend developer. It is a great opportunity to be part of the corporate team, collaborate with colleagues and provide a service for the company to earn more experiences .'
  },
  {
    Icon: PeopleSvg,
    title: 'Outsourcing',
    content:
      'On our entire career we know how to value and evaluate our colleagues, few steps away from my starting point we already encounter professional developers and engineers that has a lot of potential on their field and having a circle connection with concrete relationship to them are great opportunity, especially to those people who are great enthusiasm.'
  }
];

export default services;
